...
export default function ADUTCLTVStatus() {
  ...
  useEffect(() => {
    const fetchTVState = async () => {
      try {
        const response = await fetch('/api/adu/tcl-tv');
        const data = await response.json();
        setTvState(data);
      } catch (err) {
        console.error('Failed to fetch TCL TV state:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const togglePower = async () => {
    try {
      ...
      setTimeout(
        () =>
          fetch('/api/adu/tcl-tv')
            .then((r) => r.json())
            .then(setTvState),
        2000
      );
    } catch (err) {
      console.error('Failed to toggle TCL TV:', err);
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <h2 className="text-2xl font-bold mb-4 text-gray-900">📺 TCL TV</h2>
